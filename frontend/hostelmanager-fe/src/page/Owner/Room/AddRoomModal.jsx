...
import { X, Upload, PlusCircle, Loader, Info } from "lucide-react";
import axios from "axios";
import { uploadToCloudinary } from "../../../Config/UploadToCloudinary";

export default function AddRoomModal({ onClose, onAddRoom, token }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call parent function to add the room
  ...
  const conditions = ["New", "Good", "Average", "Needs Renovation"];
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white p-4 border-b border-gray-200 flex justify-between items-center z-10 rounded-t-xl">
          <h2 className="text-xl font-semibold text-gray-800">Thêm Phòng Mới</h2>
          <button
            onClick={onClose}
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-end mt-8 border-t pt-6">
            <div className="flex items-center text-amber-600 bg-amber-50 px-3 py-2 rounded-lg">
              ...
              <span className="text-sm">
                Phòng sẽ được gửi yêu cầu duyệt đến Admin sau khi tạo
              </span>
            </div>
            <button
              type="button"
